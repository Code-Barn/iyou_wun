...
- **Responsive 3-Column Feed Shell** — Desktop feed renders center stream + right **Discovery Rail** (`_feed_right_rail.html`) starting flush under the Layer 2 toolbar with a Mesh Relay **health widget** (click-to-expand per-relay diagnostics), TRENDING TOPICS (24H velocity tags), SOVEREIGN CREATORS (+ Follow), and an ecosystem sponsor unit. Collapses gracefully on mobile; hidden in thread mode.
